...
func (h minHeapIterator) Len() int {
	return len(h)
}
...
func (h *minHeapIterator) Push(x any) {
	*h = append(*h, x.(*heapIteratorItem))
}
...
// mergingIterator combines multiple iterators into single, sorted view.
...
func (mi *mergingIterator) findNextValid() {
	for mi.h.Len() > 0 {
		...
		mi.currentValue = currentVal
		mi.isValid = true

		if mi.lastKey.Type == OpTypeDel {
			continue
		}
...
func (mi *mergingIterator) SeekToFirst() {
	mi.h = make(minHeapIterator, 0, len(mi.h))
	heap.Init(&mi.h)
